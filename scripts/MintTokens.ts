...
async function main() {
    ...
    //Mint tokens
    const mintTx = await myTokenContract.mint(ADDRESS, TOKEN_MINT);
    await mintTx.wait();
    const totalSupplyAfter = await myTokenContract.totalSupply();
    console.log(`After minting this contract has a total supply of ${ethers.utils.formatEther(totalSupplyAfter)} tokens\n`);
    const acc1Balance = await myTokenContract.balanceOf(ADDRESS);
    console.log(`After minting ${ADDRESS} has a balance of ${ethers.utils.formatEther(acc1Balance)} tokens\n`);
    
    const votesAfterMint = await myTokenContract.getVotes(ADDRESS);
    console.log(`After minting, ${ADDRESS} has a voting power of ${votesAfterMint} votes\n`);
